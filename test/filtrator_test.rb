...
class FiltratorTest < ActiveSupport::TestCase
  test "it takes a collection, params and (optional) filters" do
    Chemex::Filtrator.new(Post.all, { id: 1 }, [])
  end

  test "it filters by all the filters you pass it" do
    post = Post.create!
    filter = Chemex::Filter.new(:id, :int, :id, nil)

    collection = Chemex::Filtrator.filter(
      Post.all,
      { filters: { id: post.id } },
      [filter],
    )
  ...
  test "it will not try to make a range out of a string field that includes ..." do
    ...
    filter = Chemex::Filter.new(:title, :string, :title, nil)
  ...
  test "it returns default when filter param not passed" do
    ...
    filter = Chemex::Filter.new(:body2, :scope, :body2, ->(c) { c.order(:body) })
    collection = Chemex::Filtrator.filter(Post.all, {}, [filter])
  ...
  test "it will not return default if param passed" do
    ...
    filter = Chemex::Filter.new(:body2, :scope, :body2, nil)
    collection = Chemex::Filtrator.filter(
      Post.all,
      { filters: { body2: "bar" } },
      [filter],
  ...
  test "it can sort on scopes that do not require arguments" do
    ...
    sort = Chemex::Sort.new(:body, :scope, :order_on_body_no_params)
    ...
    # nonetheless, Chemex supports it :)
    collection = Chemex::Filtrator.filter(Post.all, { sort: "-body" }, [sort])
